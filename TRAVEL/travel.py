...
st.set_page_config(
    page_title="AI Travel Planner",  # Title on the browser tab
    page_icon="🗺️",  # Favicon (can be an emoji or URL)
    layout="wide",  # "wide" for full-screen layout, "centered" for standard
...
# Currency selector
currencies = ['USD', 'EUR', 'GBP', 'JPY', 'AUD','INR']  # Add more currencies as needed
selected_currency = st.sidebar.selectbox('Select Currency', currencies)

# Additional user preferences
st.sidebar.subheader('Your Preferences:')
language_preference = st.sidebar.selectbox('Language Preference', ['English', 'Spanish', 'French', 'German', 'Japanese','Hindi'], index=0)
...
def get_personalized_travel_plan(user_preferences, trip_details, api_key, selected_currency):
    ...
    message = (
        ...
        f"Must-visit landmarks include {user_preferences['must_visit_landmarks']}. Also, provide a travel checklist relevant to the destination and their duration"
    )
